...
This must be the path to the feature ARG document: `.claude/arg/feature/[filename].md`
...
2. **Read Initial ARG** (for project context):
   ```bash
   Read .claude/arg/initial/[initial-arg-file].md
   ```
...
1. **Read PRP template**:
   ```bash
   Read .claude/prp/feature/template/feature-prp-template.md
   ```
...
2. **Save**:
   ```
   Location: .claude/prp/feature/[filename].md
   ```
...
```
✅ Feature PRP generated: .claude/prp/feature/[filename].md
...
Next step:
/execute-feature .claude/prp/feature/[filename].md
...
**Generated from**: .claude/arg/feature/003-email-notifications-feature-arg.md
